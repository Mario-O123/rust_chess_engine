...
use rust_chess_engine::evaluation::{ClassicalEval, Evaluator};
...
fn main() {
    ...
    println!(
        "terminal_promo — commands: help | eval | go [depth N|time MS|nodes N] | undo | undo2 | new | engine on/off | quit"
    );
    loop {
        cli.print_position();
