...
use crate::position::{Position, Cell, Color};
...
pub fn gen_pawn_moves(position: &Position, moves: &mut Vec<Move>, square: usize) {
        ...
        match piece.color {
        Color::White => {
        if pawn_starts_white.contains(&square) && position.board[(square as i32 +10) as usize]== Cell::Empty && 
        ...
        Color::Black =>  {
        if pawn_starts_black.contains(&square) && position.board[(square as i32 -10) as usize ]== Cell::Empty && 
        position.board[(square as i32 -20) as usize ]== Cell::Empty{
            //push 2 up move to vector
        ...
        if position.en_passant_square.is_some() {
            if let Some(en_passant_to) = position.en_passant_square {
                ...
                match position.player_to_move {
                    ...
                    Color::White => {
                        if (square as i32  - 9) as usize == en_passant_to || (square as i32 - 11) as usize == en_passant_to {
...
fn gen_all_promotion_pieces(square : usize , moves: &mut Vec<Move>, offset : i32 ){
         moves.push(Move::new_promotion(square , (square as i32 + offset) as usize, PromotionPiece::Knight));
         moves.push(Move::new_promotion(square , (square as i32 +offset) as usize, PromotionPiece::Bishop));
         moves.push(Move::new_promotion(square , (square as i32 +offset) as usize, PromotionPiece::Rook));
         moves.push(Move::new_promotion(square , (square as i32 +offset) as usize, PromotionPiece::Queen));
    }